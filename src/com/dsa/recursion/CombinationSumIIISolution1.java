...
public class CombinationSumIIISolution1 {

    // Method to find combinations of k numbers that add up to n
    public List<List<Integer>> combinationSum3(int k, int n) {
        List<List<Integer>> result = new ArrayList<>();
        // Initial list with numbers 1 to 9
        ArrayList<Integer> list = new ArrayList<>(Arrays.asList(1, 2, 3, 4, 5, 6, 7, 8, 9));
        int index = 0;
        ArrayList<Integer> currentList = new ArrayList<>();
        combinationSum3(list, k, n, index, currentList, result); // Call helper method
        return result;
    }

    // Helper method to find combinations recursively
    private void combinationSum3(ArrayList<Integer> list, int k, int n,
        ...
        if (k < 0 || n < 0) { // If k or n becomes negative
            return;
        }

        for (int i = index; i < list.size(); i++) {
            currentList.add(list.get(i)); // Add the number to the current combination
            // Recursively find combinations with reduced k, reduced n, and next index
            combinationSum3(list, k - 1, n - list.get(i), i + 1, currentList, result);
    ...
    public static void main(String[] args) {
        CombinationSumIIISolution1 solution = new CombinationSumIIISolution1();
