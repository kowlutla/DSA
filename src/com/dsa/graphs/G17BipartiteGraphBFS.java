...
public class G17BipartiteGraphBFS {
	...
	/**
	 * Checks if the given graph represented by an adjacency list is bipartite using Breadth First Search (BFS).
	 * 
	...
	public static boolean isBipartite(int V, ArrayList<ArrayList<Integer>> adj) {
		...
		for (int i = 0; i < V; i++) {
			if (color[i] == -1) { // If the vertex is unvisited
				if (!bfs(adj, color, i)) { // Perform BFS traversal
	...
	/**
	 * Breadth First Search (BFS) traversal to assign colors to vertices and check for conflicts.
	 * 
	 * @param adj   The adjacency list representing the graph.
	 * @param color Array to store colors of vertices.
	 * @param src   The source vertex for BFS traversal.
	 * @return      True if the graph is bipartite from the given source vertex, otherwise false.
	 */
	private static boolean bfs(ArrayList<ArrayList<Integer>> adj, int[] color, int src) {
		...
		while (!q.isEmpty()) {
			...
			for (int adjNode : adj.get(node)) {
				if (color[adjNode] == -1) { // If the adjacent vertex is unvisited
					...
					q.add(adjNode); // Enqueue the adjacent vertex
