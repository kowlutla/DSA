 ...
 *
 */
public class G31DijkstrasShortestPathPQ {

    // A class to represent a pair (weight, node) used in the priority queue.
    ...
    /**
     * Implements Dijkstra's algorithm using a priority queue to find the shortest paths from a source node to all other nodes.
     *
        ...
        while (!q.isEmpty()) {
            ...
            for (Pair adjNodePair : adj.get(currentNode)) {
                ...
                if (currentWeight + edgeWeight < distance.get(adjNode)) {
                    ...
                    q.add(new Pair(distance.get(adjNode), adjNode)); // Update the priority queue with the new distance.
                }
            }
        }
