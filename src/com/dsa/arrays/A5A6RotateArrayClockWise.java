 ...
 *
 */
public class A5A6RotateArrayClockWise {
    ...
    private static void makeOneShift(int[] arr) {
        int temp = arr[arr.length-1]; // Store the last element temporarily
        for (int i = arr.length-1; i>0; i--) { // Iterate through the array
            arr[i] = arr[i - 1]; // Move each element one position to the right
        }
        arr[0] = temp; // Place the stored last element at the beginning of the array
    }
    ...
    public static void rotateArr1(int arr[], int d, int n) {
        ...
        reverse(arr, 0, d - 1); // Reverse the first part of the array
        reverse(arr, d, n - 1); // Reverse the second part of the array
    }
